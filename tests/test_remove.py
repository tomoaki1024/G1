...
def test_to_dropbox_path_keeps_posix_path() -> None:
    assert remove._to_dropbox_path("/docs/report.txt") == "/docs/report.txt"


def test_to_dropbox_path_converts_windows_separator() -> None:
    assert remove._to_dropbox_path(r"\docs\report.txt") == "/docs/report.txt"


def test_remove_shared_link_returns_1_when_not_authenticated(monkeypatch, capsys) -> None:
...
def test_remove_shared_link_revokes_first_found_link(monkeypatch, capsys) -> None:
    dbx = SimpleNamespace()
...
def test_remove_shared_link_returns_1_when_link_is_not_found(monkeypatch, capsys) -> None:
    dbx = SimpleNamespace()
...
def test_remove_shared_link_returns_1_when_dropbox_api_fails(monkeypatch, capsys) -> None:
    dbx = SimpleNamespace()
...
def test_parser_registers_remove_subcommand() -> None:
    parser = build_parser()
    args = parser.parse_args(["remove", "folder/file.txt"])

    assert args.command == "remove"
    assert args.filepath == "folder/file.txt"
    assert args.func is remove.remove_shared_link
